...
class ColabGDrive:
  ...
  def copy_to(self, local_file=None, google_dir='root', over_write=True):
    ...
    if local_dir:
      raise FileNotFoundError('copy_to only supports copying file from the cwd.  Intermediate directory structures are not supported ' + pformat(local_dir) + ' ' + pformat(short_file_name))
